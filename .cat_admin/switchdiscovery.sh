...
BACKUP_DIR="${BACKUP_DIR:-/var/lib/tftpboot/cat}"
...
, ]+).*/\1/')"
  ...
  local -a mb_arr=()
  while IFS= read -r line; do mb_arr+=("$line"); done < <(parse_mb_serials "$outn" || true)

  local -a mac_arr=()
  while IFS= read -r line; do mac_arr+=("$line"); done < <(parse_stack_member_macs "$outn" || true)
  ...
  if [[ "$is_stack_flag" == "true" && "$stack_members_val" -gt 1 ]]; then
    ...
    if ((${#mb_arr[@]} > 0)); then
      ...
      local -a limited=()
      local i
      for ((i=0; i<stack_members && i<${#uniq[@]}; i++)); do limited+=("${uniq[i]}"); done
      if ((${#limited[@]} > 0)); then
    ...
    local first_serial=""
    if ((${#mb_arr[@]} > 0)); then first_serial="${mb_arr[0]}"; else first_serial="$sn"; fi
    ...
    local first_mac=""
    if ((${#mac_arr[@]} > 0)); then first_mac="${mac_arr[0]}"; else first_mac="$base_mac"; fi
    if [[ -n "$first_mac" ]]; then
      stack_macs_json="$(printf '%s\n' "$first_mac" | jq -R . | jq -s '.')"
    else
      stack_macs_json='[]'
...
do_selection_dialog() {
  ...
  while IFS=$US read -r ip host pid cur blacklisted bl_reason ssh login; do
    ip="${ip:-}"; [[ -n "$ip" ]] || continue
    ...
    jq -r --arg us "$US" '
      ...
      [ .[] | {
          ...
          pid: (.pid // "UNKNOWN"),
          cur: (.version // "UNKNOWN"),
          blacklisted: ((.blacklisted // false)|tostring),
          bl_reason: (.blacklist_reason // ""),
          ssh: ((.ssh // false)|tostring),
          login: ((.login // false)|tostring)
        }
      ]
      | sort_by((.blacklisted=="true"), (ipkey(.ip)))
      | .[]
      | [
          .ip, .hostname, .pid, .cur,
          .blacklisted, .bl_reason, .ssh, .login
        ] | join($us)
    ' "$disc"
  )
  ...
  if read -r lines cols < <(stty size 2>/dev/null); then :; else lines=30; cols=180; fi
  dlg_h=$(( lines - 4 ))
  dlg_w=$(( cols - 4 ))
  dlg_list=$(( dlg_h - 7 ))
  (( dlg_h < 20 )) && dlg_h=20
...
(meraki-user exists, login failed, ssh closed/unreachable, etc.)" 9 90
    sleep 3
    return 3
  fi
