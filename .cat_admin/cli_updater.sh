...
#
# Runs from: /root/.cat_admin
# Reads:
#   /root/.cat_admin/meraki_discovery.env
#   /root/.cat_admin/discovery_results.json
# Writes:
#   /root/.cat_admin/runs/cli_runs/cli-<timestamp>/
#
# Features:
...
# ---------------------------------------------------------------------------
# Force runtime root to /root/.cat_admin (no matter where script is located)
# ---------------------------------------------------------------------------
CAT_ROOT="/root/.cat_admin"
cd "$CAT_ROOT" 2>/dev/null || {
  echo "ERROR: Cannot cd to $CAT_ROOT. Create it or fix permissions." >&2
  exit 1
}
...
BASE_ENV="$CAT_ROOT/meraki_discovery.env"
DISC_JSON="$CAT_ROOT/discovery_results.json"

# runs/cli_runs/cli-YYYYmmddHHMMSS
RUNS_ROOT="$CAT_ROOT/runs/cli_runs"
