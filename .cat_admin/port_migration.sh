...
CLOUD_ADMIN_BASE="${CLOUD_ADMIN_BASE:-/root/.cat_admin}"
MERAKI_MEMORY_DIR="${MERAKI_MEMORY_DIR:-${CLOUD_ADMIN_BASE}/meraki_memory}"
MAPPING_JSON_FILE="${MAPPING_JSON_FILE:-${CLOUD_ADMIN_BASE}/meraki_switch_map.json}"

DISCOVERY_RESULTS_FILE="${DISCOVERY_RESULTS_FILE:-${CLOUD_ADMIN_BASE}/discovery_results.json}"
BACKUP_LOCAL_BASE_DIR="${BACKUP_LOCAL_BASE_DIR:-/var/lib/tftpboot/cat}"
...
# Uplink override handoff from uplink_suggest wrapper / engine.
# Default behavior:
#   - if enabled and the manifest exists, remapped uplink member ports
#     will use the source interface config recorded in final_pairs
#   - if no manifest exists, the script behaves exactly as before
UPLINK_OVERRIDE_ENABLED="${UPLINK_OVERRIDE_ENABLED:-1}"
UPLINK_OVERRIDE_MANIFEST="${UPLINK_OVERRIDE_MANIFEST:-${CLOUD_ADMIN_BASE}/runs/uplink_suggest/latest/normalized_manifest.json}"
...
load_migrate_context() {
  ...
  local PORT_MIG_LATEST_ENV="${PORT_MIG_ROOT}/latest.env"
...
Run the mapping wizard first so this module knows which source switches map to which Meraki targets." 14 90
    return 1
  fi
  ...
  SEL_IPS_RAW="$(jq -r '
    ...
    | unique
    | join(" ")
  ' "$MAPPING_JSON_FILE" 2>/dev/null || true)"
...
Run the mapping wizard and save mappings first." 14 90
    return 1
  fi
...
# ============================================================
# Mapping helpers (source IP/member -> target Cloud ID/model)
# ============================================================

get_mapping_member_count_for_ip() {
  local ip="$1"
  need jq || return 1
  [[ -f "$MAPPING_JSON_FILE" ]] || return 1

  jq -r --arg ip "$ip" '
    [ .[]?
      | select((.source.ip // "") == $ip)
      | ((.source.member_index // 1) | tonumber)
    ]
    | unique
    | length
...
find_meraki_identity_from_mapping_for_ip_member() {
  local ip="$1"
  local member="${2:-1}"
...
#   "cloud_id|model"
# Primary source: meraki_switch_map.json
# Fallback source: legacy meraki_memory/*.json
find_meraki_identity_for_ip_member() {
  local ip="$1"
  local member="${2:-1}"

  local mapped
  mapped="$(find_meraki_identity_from_mapping_for_ip_member "$ip" "$member" 2>/dev/null || true)"
  mapped="$(trim "$mapped")"
  if [[ -n "$mapped" ]]; then
    printf '%s
' "$mapped"
...
merge_ios_intent_with_meraki_ports() {
  ...
  declare -A IOS_INTENT_BY_INTERFACE=()
  while IFS=$'\t' read -r ifname row; do
    [[ -n "$ifname" && -n "$row" ]] || continue
    IOS_INTENT_BY_INTERFACE["$ifname"]="$row"
  done < <(
    jq -cr '
      select(type=="object")
      | select((.interface? // null) | type=="string")
  ...
  declare -A UPLINK_OVERRIDE_BY_TARGET_PORTID=()
  declare -A UPLINK_OVERRIDE_BY_TARGET_INTERFACE=()

  if [[ "${UPLINK_OVERRIDE_ENABLED:-1}" == "1" && -f "${UPLINK_OVERRIDE_MANIFEST:-}" ]]; then
    while IFS=$'\t' read -r target_portid target_interface source_interface; do
      [[ -n "$source_interface" ]] || continue
      if [[ -n "$target_portid" ]]; then
        UPLINK_OVERRIDE_BY_TARGET_PORTID["$target_portid"]="$source_interface"
      fi
      if [[ -n "$target_interface" ]]; then
        UPLINK_OVERRIDE_BY_TARGET_INTERFACE["$target_interface"]="$source_interface"
      fi
    done < <(
      jq -r --arg ip "$ip" --arg m "$member" '
        .[]?
        | select((.ip|tostring) == $ip and (.member_index|tostring) == $m)
        | (.final_pairs // [])[]
        | select((.source // "") != "")
        | [
        ...
        | @tsv
      ' "$UPLINK_OVERRIDE_MANIFEST" 2>/dev/null || true
    )

    if [[ "${#UPLINK_OVERRIDE_BY_TARGET_PORTID[@]}" -gt 0 || "${#UPLINK_OVERRIDE_BY_TARGET_INTERFACE[@]}" -gt 0 ]]; then
      echo "INFO: Loaded uplink override manifest for IP=$ip member=$member from $UPLINK_OVERRIDE_MANIFEST" >>"$build_log"
      for key in "${!UPLINK_OVERRIDE_BY_TARGET_PORTID[@]}"; do
        echo "  OVERRIDE targetPortId=$key -> source=${UPLINK_OVERRIDE_BY_TARGET_PORTID[$key]}" >>"$build_log"
      done
      for key in "${!UPLINK_OVERRIDE_BY_TARGET_INTERFACE[@]}"; do
        echo "  OVERRIDE targetInterface=$key -> source=${UPLINK_OVERRIDE_BY_TARGET_INTERFACE[$key]}" >>"$build_log"
      done
    fi
  fi

  echo "Step 2: Looking up Meraki identity for IP $ip member $member" >>"$build_log"
  apply_ui_update "Looking up Meraki device identity (mapping JSON)" 25

  local id_line cloud_id model
  id_line="$(find_meraki_identity_for_ip_member "$ip" "$member" 2>>"$build_log" || true)"
  if [[ -z "$id_line" ]]; then
    echo "ERROR: No mapping entry found for IP $ip member $member" >>"$build_log"
    echo "No mapping entry found for IP $ip member $member – cannot fetch ports." >&2
    return 1
  fi
  ...
  while IFS= read -r meraki_port; do
    ...
    local ios_entry override_source_if target_if_guess
    ios_entry=""
    override_source_if=""
    target_if_guess=""
    ...
    if [[ "$pid" =~ ^[0-9]+$ ]]; then
      ...
      jq -c --arg pid "$pid" --arg m "$member" '
        def mem_of_if(i):
          ...
          elif i | test("^[A-Za-z]+0/[0-9]+$") then
            "1"
          elif i | test("^[A-Za-z]+[0-9]+/[0-9]+$") then
            (
              i
              | capture("^[A-Za-z]+(?<m>[0-9]+)/(?<p>[0-9]+)$").m
              | if . == "0" then "1" else . end
            )
          elif i | test("^[A-Za-z]+[0-9]+/0/[0-9]+$") then
            (
              i
              | capture("^[A-Za-z]+(?<m>[0-9]+)/0/[0-9]+$").m
              | if . == "0" then "1" else . end
            )
          elif i | test("^[A-Za-z]+[0-9]+/1/[0-9]+$") then
            (
              i
              | capture("^[A-Za-z]+(?<m>[0-9]+)/1/[0-9]+$").m
              | if . == "0" then "1" else . end
            )
          else "" end;
    ...
    if [[ -z "$ios_entry" && "$pid" =~ ^[0-9]+_[^_]+_[0-9]+$ ]]; then
      override_source_if="${UPLINK_OVERRIDE_BY_TARGET_PORTID[$pid]:-}"

      if [[ -z "$override_source_if" ]]; then
        target_if_guess="$(jq -r '.name // ""' <<<"$meraki_port" 2>/dev/null || echo "")"
        target_if_guess="$(trim "$target_if_guess")"
        if [[ -n "$target_if_guess" && -n "${UPLINK_OVERRIDE_BY_TARGET_INTERFACE[$target_if_guess]:-}" ]]; then
          override_source_if="${UPLINK_OVERRIDE_BY_TARGET_INTERFACE[$target_if_guess]}"
        fi
      fi

      if [[ -n "$override_source_if" && -n "${IOS_INTENT_BY_INTERFACE[$override_source_if]:-}" ]]; then
        ios_entry="${IOS_INTENT_BY_INTERFACE[$override_source_if]}"
        echo "  OVERRIDE-APPLY: portId=$pid source_interface=$override_source_if" >>"$build_log"
      elif [[ -n "$override_source_if" ]]; then
        echo "  OVERRIDE-MISS: portId=$pid source_interface=$override_source_if not found in parsed IOS intent" >>"$build_log"
      fi
    fi
...
run_auto_build_then_apply_from_checklist() {
  ...
  dlg --backtitle "$BACKTITLE_PORTS" \
      ...
      --msgbox "Completed automated build+apply.\n\nOK:   $ok\nFAIL: $fail\n\n(FAIL usually means: missing backup config, missing mapping identity, or no diff built.)" \
      14 70
