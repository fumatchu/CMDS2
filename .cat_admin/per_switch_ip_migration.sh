...
# ============================================================
# Cat Admin – Mgmt IP Migration (IOS cfg -> Meraki Dashboard)
#
# What it does:
...
#   - Prompts operator for the management VLAN ID
#   - For each selected management IP:
#       * Prefer exact backup_filename from selected_upgrade.json / discovery_results.json
#       * Else find latest IOS cfg in /var/lib/tftpboot/cat based on hostname / pid prefix
#       * Parse interface Vlan<mgmt_vlan>
...
#           - Update Meraki device managementInterface using cloud_id(s) from meraki_switch_map.json
...
#   - Logs to /root/.cat_admin/runs/mgmt_ip/<run_id>/actions.log
#   - latest symlink points to the most recent run dir
#   - Dialog:
#       * Management VLAN input
#       * Checklist selection screen
#       * Gauge progress while processing
#       * Final summary shows counts only (NO file paths)
# ============================================================

CAT_ADMIN_ROOT="/root/.cat_admin"
IOS_CFG_DIR="/var/lib/tftpboot/cat"
DISC_ENV="${CAT_ADMIN_ROOT}/meraki_discovery.env"
DISCOVERY_JSON="${CAT_ADMIN_ROOT}/discovery_results.json"
SELECTED_UPGRADE_ENV="${CAT_ADMIN_ROOT}/selected_upgrade.env"
SELECTED_UPGRADE_JSON="${CAT_ADMIN_ROOT}/selected_upgrade.json"
MERAKI_SWITCH_MAP_JSON="${CAT_ADMIN_ROOT}/meraki_switch_map.json"

: "${DIALOG:=dialog}"
: "${BACKTITLE:=Cat Admin – Mgmt IP Migration}"
: "${TITLE:=Mgmt IP Migration}"

RUNS_ROOT="${CAT_ADMIN_ROOT}/runs/mgmt_ip"
...
prompt_for_mgmt_vlan() {
  local val
  val="$(dlg --inputbox "Enter the management VLAN ID to inspect in the IOS config.\n\nExample: 245" 11 60 "1")" || {
    log "User cancelled management VLAN prompt."
    exit 0
  }

  val="$(printf '%s' "$val" | tr -d '\r' | xargs)"
  [[ -n "$val" ]] || die "Management VLAN ID cannot be empty."
  [[ "$val" =~ ^[0-9]+$ ]] || die "Management VLAN ID must be numeric."
  (( val >= 1 && val <= 4094 )) || die "Management VLAN ID must be between 1 and 4094."

  MGMT_VLAN_ID="$val"
  log "Using management VLAN ID: $MGMT_VLAN_ID"
}
...
cloud_ids_for_ip() {
  local ip="$1"
  python3 - "$MERAKI_SWITCH_MAP_JSON" "$ip" <<'PY'
import sys, json
map_path, target_ip = sys.argv[1], sys.argv[2]

try:
  rows = json.load(open(map_path, "r"))
except Exception:
  sys.exit(1)
...
for row in rows:
  ...
  if str(src.get("ip","")).strip() != target_ip:
    continue
  cid = str(tgt.get("cloud_id","") or "").strip()
  if not cid:
    continue
  try:
    mi = int(src.get("member_index"))
  except Exception:
    mi = 9999
  hits.append((mi, cid))
...
backup_filename_for_ip() {
  local ip="$1"
  python3 - "$SELECTED_UPGRADE_JSON" "$DISCOVERY_JSON" "$ip" <<'PY'
import sys, json, os
sel_path, disc_path, target_ip = sys.argv[1:4]
...
for rows in (load_list(sel_path), load_list(disc_path)):
    for row in rows:
        if str(row.get("ip","")).strip() != target_ip:
            continue
        fn = str(row.get("backup_filename","") or "").strip()
        if fn:
            print(fn)
            sys.exit(0)
sys.exit(1)
...
find_cfg_for_ip_from_index() {
  ...
  backup_filename="$(backup_filename_for_ip "$ip" 2>/dev/null || true)"
  if [[ -n "$backup_filename" && -f "${IOS_CFG_DIR}/${backup_filename}" ]]; then
    echo "${IOS_CFG_DIR}/${backup_filename}"
    return 0
  fi
  ...
  pid="$(normalize_name_for_cfg_prefix "$pid")"
...
build_switch_index_tsv() {
  python3 - "$SELECTED_UPGRADE_JSON" "$DISCOVERY_JSON" "$MERAKI_SWITCH_MAP_JSON" "$SELECTED_UPGRADE_ENV" "$SWITCH_INDEX_TSV" <<'PY'
import json, os, sys

sel_json, disc_json, map_json, sel_env, out_path = sys.argv[1:6]
...
rows_src = load_list(sel_json)
if not rows_src:
  rows_src = load_list(disc_json)

map_rows = load_list(map_json)

def primary_cloud_and_stackcount(ip):
  hits=[]
  for row in map_rows:
    src = row.get("source") or {}
    tgt = row.get("target") or {}
    if str(src.get("ip","")).strip() != ip:
      continue
    cid = str(tgt.get("cloud_id","") or "").strip()
    if not cid:
      continue
...
with open(out_path, "w") as f:
  for ip in selected:
    ...
    primary_cid, stack_count = primary_cloud_and_stackcount(ip)
...
select_switches_dialog_checklist() {
  ...
  while IFS=$'\t' read -r ip hn dn pid primary_cid stack_count; do
    ...
    if [[ -n "${primary_cid:-}" ]]; then
      ...
      cid_part="Switch QTY:${stack_count:-0}"
    fi
  ...
  dlg --separate-output --checklist "$prompt" 22 115 14 "${menu[@]}"
}
...
PY
      ...
      echo "XXX"; echo "$((base + span*15/100 + 1))"; echo "[$idx/$total] Finding latest IOS cfg"; echo "XXX"
      local cfg=""
      cfg="$(find_cfg_for_ip_from_index "$ip" || true)"
      if [[ -z "$cfg" ]]; then
        log "SKIP: could not find cfg for $ip by backup filename / hostname / device_name / pid prefixes"
        python3 - "$ip" <<'PY' >>"$items_json"
import json,sys
...
PY
      ...
      if [[ -z "$cloud_list" ]]; then
        log "SKIP: no meraki_switch_map mapping found for IP $ip (cloud_id unknown)"
...
PY
      ...
      echo "XXX"; echo "$((base + span*45/100 + 1))"; echo "[$idx/$total] Parsing mgmt VLAN Vlan${MGMT_VLAN_ID}"; echo "XXX"
      local mgmt_mode
      mgmt_mode="$(parse_vlan_mgmt_mode_from_cfg "$cfg" "$MGMT_VLAN_ID" | tr -s ' ' | trim || true)"

      if [[ -z "$mgmt_mode" || "$mgmt_mode" == "DHCP" ]]; then
        log "DHCP: no static mgmt IP for Vlan${MGMT_VLAN_ID} (no change)"
        while IFS= read -r cid; do
          [[ -n "$cid" ]] || continue
...
PY
  ...
  python3 - "$RUN_ID" "${RUN_DIR}/items.jsonl" "${RUN_DIR}/stats.env" "$REPORT_JSON" "$MGMT_VLAN_ID" <<'PY'
import json,sys
run_id, items_path, stats_path, out_path, mgmt_vlan = sys.argv[1:6]
items=[]
...
doc={
  "run_id": run_id,
  "management_vlan_id": int(mgmt_vlan),
...
main() {
  ...
  [[ -f "$DISCOVERY_JSON" ]] || die "Missing ${DISCOVERY_JSON}"
  [[ -f "$MERAKI_SWITCH_MAP_JSON" ]] || die "Missing ${MERAKI_SWITCH_MAP_JSON}"
  ...
  build_switch_index_tsv >/dev/null 2>&1 || die "Could not build index (check selected_upgrade.env + selected_upgrade.json / discovery_results.json / meraki_switch_map.json)."
