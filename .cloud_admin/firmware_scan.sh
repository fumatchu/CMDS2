...
write_report_env() {
  umask 077
  mkdir -p "$(dirname "$REPORT_ENV_FILE")" 2>/dev/null || true
  {
  ...
  chmod 600 "$REPORT_ENV_FILE" 2>/dev/null || true
}
...
run_collection() {
  ...
  if (( DIALOG_AVAILABLE )); then
    ...
    echo "  $REPORT_ROOT/latest -> $(readlink -f "$REPORT_ROOT/latest" 2>/dev/null || true)"
    echo "  $REPORT_ROOT/latest.json -> $(readlink -f "$REPORT_ROOT/latest.json" 2>/dev/null || true)"
  fi
