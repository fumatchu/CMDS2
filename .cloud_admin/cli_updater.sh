...
#
# Runs from: /root/.cloud_admin
# Reads:
#   /root/.cloud_admin/meraki_discovery.env
#   /root/.cloud_admin/discovery_results.json
# Writes:
#   /root/.cloud_admin/runs/cli_runs/cli-<timestamp>/
#
# Features:
...
# ---------------------------------------------------------------------------
# Force runtime root to /root/.cloud_admin (no matter where script is located)
# ---------------------------------------------------------------------------
CLOUD_ROOT="/root/.cloud_admin"
cd "$CLOUD_ROOT" 2>/dev/null || {
  echo "ERROR: Cannot cd to $CLOUD_ROOT. Create it or fix permissions." >&2
  exit 1
}
...
BASE_ENV="$CLOUD_ROOT/meraki_discovery.env"
DISC_JSON="$CLOUD_ROOT/discovery_results.json"

# runs/cli_runs/cli-YYYYmmddHHMMSS
RUNS_ROOT="$CLOUD_ROOT/runs/cli_runs"
...
run_cli_for_ip(){
  ...
  expect -f - <<'EXP' >"$outfile_raw" 2>&1
    ...
    if {$used_enable} { exit 10 } else { exit 0 }
EXP
  rc=$?
  set -e
