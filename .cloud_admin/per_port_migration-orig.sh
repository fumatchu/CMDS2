...
build_port_diff_for_one_switch() {
  ...
  if (( failures > 0 )); then
    dlg --backtitle "$BACKTITLE_PORTS" \
        --title "Stack diffs built (partial)" \
        --msgbox "Built diffs for $built stack member(s) on IP $ip, but $failures member(s) failed or had no meraki_memory entry.\n\nOutputs:\n  $RUN_DIR/ports\n\nLook for files with:\n  ${ip}_${base_name}_m1_...\n  ${ip}_${base_name}_m2_...\n  etc." 16 90
    return 1
  fi
...
apply_port_diff_for_one_switch() {
  ...
  if [[ "$member_count" -le 1 ]]; then
    ...
    member="$(pick_stack_member_for_ip "$ip")" || return 1
    member="$(trim "$member")"
    [[ -z "$member" ]] && member="1"
  ...
  local diff_file
  diff_file="$(find_latest_ports_diff_for_ip_member "$ip" "$member" 2>/dev/null || true)"
  diff_file="$(trim "$diff_file")"

  if [[ -z "$diff_file" ]]; then
    dlg --backtitle "$BACKTITLE_PORTS" \
        --title "No diff found" \
        --msgbox "Could not find a ports_diff.json file for:\n\n  IP:     $ip\n  Member: $member\n\nLooked under:\n  $RUN_DIR/ports\n\nRun \"Build port diff\" first." 15 90
    return 1
  fi

    apply_ports_from_diff "$ip" "$diff_file" "$member" "SINGLE"
}
