...
# Dialog-based viewer for backup configs in /var/lib/tftpboot/mig
...
list_cfg_files() {
  # Non-recursive, *.cfg only, sorted (natural sort)
  ...
  while true; do
    ...
    case "$rc" in
      0)  # Yes -> loop and ask again
          continue
          ;;
      1|255) # No / ESC -> go back to file list
          return 1
          ;;
    esac
  done
