...
#
# Logging (v2 + v1-style per-phase logs):
#   Standalone runs ( ./script preflight | fix-dns | fix-ntp | fix-aaa | fix-routing ):
#     runs/<phase>fix/<runid>/ui.status              (narrative log — same as v1)
#     runs/<phase>fix/<runid>/<phase>fix.csv
#     runs/<phase>fix/<runid>/devlogs/<ip>.*.log
#
#   Full pipeline ( ./script all ):
#     runs/pipeline/pipeline-<id>/ui.status          (MASTER log — every line, all phases)
#     runs/pipeline/pipeline-<id>/preflight/ui.status   (per-phase narratives)
#     runs/pipeline/pipeline-<id>/dnsfix/{ui.status, dnsfix.csv, devlogs/}
#     runs/pipeline/pipeline-<id>/ntpfix/{ui.status, ntpfix.csv, devlogs/}
#     runs/pipeline/pipeline-<id>/aaafix/{ui.status, aaafix.csv, devlogs/}
#     runs/pipeline/pipeline-<id>/iprfix/{ui.status, iprfix.csv, devlogs/}
#
...
STATUS_FILE=""      # master tailbox file (driven by --tailboxbg)
PHASE_STATUS=""     # per-phase narrative log file (set by set_phase_log)
GAUGE_PIPE=""; GAUGE_FD=""; DPID=""; MAIN=$$
export STATUS_FILE PHASE_STATUS
...
# log(): writes one timestamped line to the dialog tailbox AND, if set,
# to the per-phase log file. This restores v1-style per-phase logging
# while preserving v2's master pipeline log.
log(){
  local line; line="$(printf '%s %s' "$(now)" "$1")"
  if [[ -n "${STATUS_FILE:-}" ]]; then
    printf '%s\n' "$line" >> "$STATUS_FILE" 2>/dev/null || true
  fi
  if [[ -n "${PHASE_STATUS:-}" && "${PHASE_STATUS}" != "${STATUS_FILE:-}" ]]; then
    printf '%s\n' "$line" >> "$PHASE_STATUS" 2>/dev/null || true
  fi
}

# Set / clear the per-phase log file. Safe to call multiple times.
#   set_phase_log "<dir>" "<short_name>"   -> writes <dir>/ui.status
#   clear_phase_log                        -> stop teeing per-phase
set_phase_log(){
  local d="$1"
  [[ -n "$d" ]] || return 0
  mkdir -p "$d"
  PHASE_STATUS="$d/ui.status"
  : >> "$PHASE_STATUS"
  export PHASE_STATUS
  printf '%s ----- PHASE LOG OPEN: %s -----\n' "$(now)" "$PHASE_STATUS" >> "$PHASE_STATUS"
}
clear_phase_log(){
  if [[ -n "${PHASE_STATUS:-}" ]]; then
    printf '%s ----- PHASE LOG CLOSE -----\n' "$(now)" >> "$PHASE_STATUS" 2>/dev/null || true
  fi
  PHASE_STATUS=""
  export PHASE_STATUS
}

# When running the unified pipeline (meraki_all), each phase's subdirectory
# lives at runs/pipeline/pipeline-<TS>/<phase>fix/. The show_logs.sh viewer
# scans runs/<phase>fix/<runid>/ instead. To bridge that without duplicating
# artifacts, we drop a symlink at the viewer's expected location pointing at
# the pipeline subdirectory:
#
#   runs/dnsfix/dns-<TS>      -> runs/pipeline/pipeline-<TS>/dnsfix
#   runs/ntpfix/ntp-<TS>      -> runs/pipeline/pipeline-<TS>/ntpfix
#   runs/aaafix/aaa-<TS>      -> runs/pipeline/pipeline-<TS>/aaafix
#   runs/iprfix/ipr-<TS>      -> runs/pipeline/pipeline-<TS>/iprfix
#
# We also update each viewer root's "latest" symlink. Standalone runs
# (./script fix-dns etc.) continue to write directly to the viewer location
# and don't need this — they already show up.
register_phase_for_viewer() {
  local prefix="$1" pdir="$2"
  [[ -n "$prefix" && -n "$pdir" ]] || return 0
  [[ -n "${MASTER_RUN_ID:-}" ]]    || return 0
  local viewer_root="$SCRIPT_DIR/runs/${prefix}fix"
  mkdir -p "$viewer_root"
  local ts="${MASTER_RUN_ID#pipeline-}"
  local link="$viewer_root/${prefix}-${ts}"
  ln -sfn "$pdir" "$link"                2>/dev/null || true
  ln -sfn "$pdir" "$viewer_root/latest"  2>/dev/null || true
}
...
_pipeline_summary(){
  ...
  printf 'Run ID  : %s\n' "$run_id"
  printf 'Results : %s\n' "$mdir"
  printf 'Logs    : %s/ui.status (master)\n' "$mdir"
  printf '          %s/<phase>fix/ui.status (per-phase)\n' "$mdir"
  printf '\n'
...
meraki_all(){
  ...
  # ══════════════════════════════════════════════════════════════════
  set_phase_log "$MASTER_DIR/preflight"
  log "Phase log: $PHASE_STATUS"
  ...
  set_phase_log "$RUN_DIR"
  register_phase_for_viewer "dns" "$RUN_DIR"
  log "Phase log: $PHASE_STATUS"
  log "Viewer link: $SCRIPT_DIR/runs/dnsfix/dns-${MASTER_RUN_ID#pipeline-}"
  DNSFIX_CSV="$RUN_DIR/dnsfix.csv"
  echo "ip,changed_dns,enabled_domain_lookup,resolution_ok,notes" > "$DNSFIX_CSV"
  local -a DNS_TARGETS
  ...
  if [[ -z "${NTP_PRIMARY:-}" && -z "${NTP_SECONDARY:-}" ]]; then
    ...
    set_phase_log "$RUN_DIR"
    register_phase_for_viewer "ntp" "$RUN_DIR"
    log "Phase log: $PHASE_STATUS"
    log "Viewer link: $SCRIPT_DIR/runs/ntpfix/ntp-${MASTER_RUN_ID#pipeline-}"
    NTPFIX_CSV="$RUN_DIR/ntpfix.csv"
    echo "ip,changed_ntp,synced,notes" > "$NTPFIX_CSV"
    local -a NTP_TARGETS
  ...
  set_phase_log "$RUN_DIR"
  register_phase_for_viewer "aaa" "$RUN_DIR"
  log "Phase log: $PHASE_STATUS"
  log "Viewer link: $SCRIPT_DIR/runs/aaafix/aaa-${MASTER_RUN_ID#pipeline-}"
  AAAFIX_CSV="$RUN_DIR/aaafix.csv"
  echo "ip,changed_aaa,new_model,auth_login_default_local,authz_exec_default_local,notes" > "$AAAFIX_CSV"
  local -a AAA_TARGETS
  ...
  set_phase_log "$RUN_DIR"
  register_phase_for_viewer "ipr" "$RUN_DIR"
  log "Phase log: $PHASE_STATUS"
  log "Viewer link: $SCRIPT_DIR/runs/iprfix/ipr-${MASTER_RUN_ID#pipeline-}"
  IPRFIX_CSV="$RUN_DIR/iprfix.csv"
  echo "ip,mgmt_mode,changed_config,ip_routing_enabled,default_route_present,ping_8_8_8_8_ok,notes" > "$IPRFIX_CSV"
  local -a IPR_TARGETS
  ...
  log "Master log:    $STATUS_FILE"
  log "Per-phase logs under: $MASTER_DIR/<phase>fix/ui.status"
  ...
  dlg --title " Pipeline Complete — $MASTER_RUN_ID " --textbox "$_stmp" 32 78
  rm -f "$_stmp"
  clear
  trap - EXIT
}
