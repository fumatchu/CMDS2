...
BACKTITLE="Advanced IOS-XE Image Deployment (Scheduling) Module"
...
intro_text="Advanced IOS-XE Image Deployment (Scheduling) Module

This is an advanced deployment workflow that allows you to schedule
...
# All scripts live in the same bin folder as this wrapper.
run_step "Setup Wizard"      "$BASE_DIR/setupwizard.sh"
run_step "Device Discovery"  "$BASE_DIR/discoverswitches.sh"
run_step "Schedule Upgrade"  "$BASE_DIR/scheduler.sh"

dialog \
  --no-shadow \
  --backtitle "$BACKTITLE" \
  --title "Complete" \
  --msgbox "All stages completed. Upgrade has been scheduled." 7 50
