...
USAGE
  ...
  load_switches() {
    ...
    if (( ${#VAL_ENV_IPS[@]} == 0 )); then
      dlg --backtitle "$BACKTITLE_M" \
          --title "Empty validation results" \
          --msgbox "Validation results file exists but contains no validated switches.\n\nRun validation again and then re-run this tool." 12 80
      return 1
    fi
  ...
  printf -v MSG '%s\n\nOrg:\n  %s (%s)\n\nMapping JSON:\n  %s\n\nENV:\n  %s\n\nYou can source the ENV file from other scripts, for example:\n  source "%s"\n\nThe JSON includes networkAddress per switch so you can apply the\ncorrect physical address when you push configuration to each device.\n' \
    "Switch-to-network mapping saved." \
    "$ORG_NAME" "$ORG_ID" \
    "$MAP_JSON_OUT" \
    "$MAP_ENV_OUT" \
    "$MAP_ENV_OUT"
